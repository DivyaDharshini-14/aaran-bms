                        ...
                        <x-slot:table_body name="table_body">
                            @foreach(\App\Helper\Months::months() as $index=>$row)
                                <x-table.row>
                                    ...
                                    <x-table.cell-text>
                                        <button wire:click="monthlyReport({{$index+1}})"
                                                @click="setTimeout(()=>open = 2,1500)">{{\Aaran\Reports\sales\MonthlyReport::monthlySales($index+1)}}</button>
                                    </x-table.cell-text>
                                </x-table.row>
                                    <?php
                                    $totalSales += \Aaran\Reports\sales\MonthlyReport::monthlySales($index + 1);
                                    ?>
                            @endforeach
                            <x-table.row>
